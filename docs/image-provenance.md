...
| `pond-still-frame.{jpg,webp,avif}` / `pond-still-frame@2x.*` | `SkyViewSection.razor` (still-frame card), `Home.razor` JSON-LD `thumbnailUrl` | "Real footage from a live Oxyniti trial." / `FIELD TRIAL · SOUTH INDIA` / `STILL FRAME` | Real aerial/drone frame — a boat, the two units, hoses and a visible plume in a natural pond setting; replaces `drone-poster.jpg` (same clip, a frame without people) | OK |
