...
| `pond-still-frame.{jpg,webp,avif}` / `pond-still-frame@2x.*` | `Home.razor` JSON-LD `thumbnailUrl` (source frame for `pond-aerial-wide`) | "Real footage from a live Oxyniti trial." | Real aerial/drone frame — a boat, the two units, hoses and a visible plume in a natural pond setting; replaces `drone-poster.jpg` (same clip, a frame without people) | OK |
| `pond-aerial-wide.{jpg,webp,avif}` / `pond-aerial-wide@2x.*` | `SkyViewSection.razor` (landscape poster card, links to the YouTube Short) | "Real footage from a live Oxyniti trial." / `FIELD TRIAL · SOUTH INDIA` / `STILL FRAME` | Unretouched 4:3 crop (rows 260–860) of `pond-still-frame@2x.jpg` — same real drone frame, cropped to the units, hoses, boat and plume; resized only | OK |
| `plume-poster.jpg` | `FieldFootageSection.razor` (portrait still card, above the "Watch on YouTube" link to the Short) | "The nano-plume — oxygen dissolving in real time" | Real footage frame — same net-enclosure scene as the hero shot, visible turbulence from the diffuser | OK |
...
| `drone.webm` / `drone.mp4` | Not referenced anywhere in current code (the clip now plays from YouTube via `SkyViewSection.razor`) | none | Real aerial footage (matches `pond-still-frame.jpg`) | **Orphaned** — safe to delete once the YouTube link is confirmed live |
| `plume-portrait.webm` / `plume-portrait.mp4` | Not referenced anywhere in current code (the Use Cases clip now plays from YouTube via `FieldFootageSection.razor`) | none | Real footage (matches `plume-poster.jpg`); explicitly called out in #78 as reading genuine | **Orphaned** — safe to delete once the YouTube link is confirmed live |
